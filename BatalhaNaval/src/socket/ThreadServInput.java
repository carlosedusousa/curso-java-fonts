		   			...
		   			while (s.hasNextLine()) {
   							...
   							if (protocolo[0] == '$') {
   									if (this.numeroConexao == 1){
   	   										...
   	   										if (leStringProtocolo(protocolo).equals(nome)){ //CONSULTA BD
   	   												login = true;
   	   												servidor.uniCast(0,"$OK1");
   	   										}
   	   										else if (leStringProtocolo(protocolo).equals(senha)){ //CONSULTA BD
   	   											if (login){
												...
												totalJogos = 34;
  												if (contador == 0){
  													Thread.sleep(1000);
  													servidor.uniCast(0,"$"+imagem); //CONSULTA BD
  												}
  												else if (contador == 1){
  													Thread.sleep(1000);
  													servidor.uniCast(0,"$"+pais); //CONSULTA BD
  												}
  												else if (contador == 2){
  													Thread.sleep(1000);
  													servidor.uniCast(0,"$"+medalhas); //CONSULTA BD
  												}
  												else if (contador == 3){
  													Thread.sleep(1000);
  													servidor.uniCast(0,"$"+vitorias); //CONSULTA BD
  												}
  												else if (contador == 4){
  													Thread.sleep(1000);
  													servidor.uniCast(0,"$"+derrotas); //CONSULTA BD
  												}	
  												else if (contador == 5){
  													Thread.sleep(1000);
  													servidor.uniCast(0,"$"+totalJogos); //CONSULTA BD
  												}
  												else{
  													Thread.sleep(1000);
   									...
   									if (this.numeroConexao == 2){		
   										...
   										if (leStringProtocolo(protocolo).equals(nome)){
											...
											totalJogos = 20;
												if (contador == 0){
													Thread.sleep(1000);
													servidor.uniCast(1,"$"+imagem); //CONSULTA BD
												}
												else if (contador == 1){
													Thread.sleep(1000);
													servidor.uniCast(1,"$"+pais); //CONSULTA BD
												}
												else if (contador == 2){
													Thread.sleep(1000);
													servidor.uniCast(1,"$"+medalhas); //CONSULTA BD
												}
												else if (contador == 3){
													Thread.sleep(1000);
													servidor.uniCast(1,"$"+vitorias); //CONSULTA BD
												}
												else if (contador == 4){
													Thread.sleep(1000);
													servidor.uniCast(1,"$"+derrotas); //CONSULTA BD
												}	
												else if (contador == 5){
													Thread.sleep(1000);
													servidor.uniCast(1,"$"+totalJogos); //CONSULTA BD
												}
												else{
													Thread.sleep(1000);
   	   						...
   	   						// Msg do Banco de Dados (Jogador, Ranking)
   	   								...
   	   								if (this.numeroConexao == 1){
   	   										...
   	   										if (leStringProtocolo(protocolo).equals("H")){ 
   	   											servidor.uniCast(0,"Fulano - 0.59"); //CONSULTA BD
   	   											Thread.sleep(500);
   	   											servidor.uniCast(0,"Beltrano - 1.29"); //CONSULTA BD
   	   											Thread.sleep(500);
   	   											servidor.uniCast(0,"Ciclano - 2.49"); //CONSULTA BD
   	   											Thread.sleep(500);
   	   											servidor.uniCast(0,"Deltrano - 3.29"); //CONSULTA BD
   	   											Thread.sleep(500);
   	   											servidor.uniCast(0,"Teste - 4.00"); //CONSULTA BD
   	   										}
   	   										System.out.println("Solicitacao de Consulta ao BD do cliente 1: " + msg);
   	   								}	
   	   								if (this.numeroConexao == 2){		
   	   										if (leStringProtocolo(protocolo).equals("H")){
   	   											servidor.uniCast(1,"Fulano - 0.59"); //CONSULTA BD
   	   											Thread.sleep(500); 
   	   											servidor.uniCast(1,"Beltrano - 1.29"); //CONSULTA BD
   	   											Thread.sleep(500);
   	   											servidor.uniCast(1,"Ciclano - 2.49"); //CONSULTA BD
   	   											Thread.sleep(500);
   	   											servidor.uniCast(1,"Deltrano - 3.29"); //CONSULTA BD
   	   											Thread.sleep(500);
   	   											servidor.uniCast(1,"Teste - 4.00"); //CONSULTA BD
	   										}
							...
							if (player1.getVida() == 0){
										...
										player2.setVitorias(player2.getVitorias()+1); //UPDATE BD
										player2.setTotalJogos(player2.getTotalJogos()+1); //UPDATE BD
										// Atualiza informacoes de quem perdeu
										servidor.uniCast(0,"$over");
										player1.setDerrotas(player1.getDerrotas()+1); //UPDATE BD
										player1.setTotalJogos(player1.getTotalJogos()+1); //UPDATE BD
										// Atualiza Medalhas de quem ganhou
										if (player2.getVitorias()%5==0)
											player2.setMedalhas(player2.getVitorias()/5); //UPDATE BD
							}
							if (player2.getVida() == 0) {
										Thread.sleep(500);
										// Atualiza informacoes de quem ganhou
										servidor.uniCast(0,"$win");
										player1.setVitorias(player1.getVitorias()+1); //UPDATE BD
										player1.setTotalJogos(player1.getTotalJogos()+1); //UPDATE BD
										// Atualiza informacoes de quem perdeu
										servidor.uniCast(1,"$over");
										player2.setDerrotas(player2.getDerrotas()+1); //UPDATE BD
										player2.setTotalJogos(player2.getTotalJogos()+1); //UPDATE BD
										// Atualiza Medalhas de quem ganhou
										if (player2.getVitorias()%5==0)
											player2.setMedalhas(player2.getVitorias()/5); //UPDATE BD
